...
import (
	"alto_server/common/netconf2/transport"
	"context"
	"encoding/xml"
	"errors"
	"fmt"
...
func (s *Session) send(ctx context.Context, msg *request) (chan RPCReply, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Session) Do(ctx context.Context, req any) (*RPCReply, error) {
	...
	case <-time.After(timeout):
		...
		delete(s.reqs, msg.MessageID)
		s.mu.Unlock()
		return nil, fmt.Errorf("netconf reply timeout after %v", timeout)
	}
}
