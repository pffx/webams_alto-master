...
import (
	...
	"alto_server/common/netconf/message"
)
...
func (session *Session) listen() {
	go func() {
		re, err := regexp.Compile(message.RpcReplyRegex)
		if err != nil {
			fmt.Println("RpcReplyRegex compile error :", err)
			return
		}
		re2, err := regexp.Compile(message.NotificationMessageRegex)
		if err != nil {
			fmt.Println("NotificationMessageRegex compile error :", err)
			return
		}
